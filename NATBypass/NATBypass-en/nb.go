...
func forward(conn1 net.Conn, conn2 net.Conn) {
	...
	// wait tow goroutines
	wg.Add(2)
	go connCopyEn(conn1, conn2, &wg)
	go connCopyDe(conn2, conn1, &wg)
	//blocking when the wg is locked
	wg.Wait()
}

func connCopyEn(conn1 net.Conn, conn2 net.Conn, wg *sync.WaitGroup) {
	nccrypt.CopyToEncrypt(conn1, conn2)
	conn1.Close()
	log.Println("[←]", "close the connect at local:["+conn1.LocalAddr().String()+"] and remote:["+conn1.RemoteAddr().String()+"]")
	wg.Done()
}
func connCopyDe(conn1 net.Conn, conn2 net.Conn, wg *sync.WaitGroup) {
	nccrypt.CopyFromDecrypt(conn1, conn2)
	conn1.Close()
	log.Println("[←]", "close the connect at local:["+conn1.LocalAddr().String()+"] and remote:["+conn1.RemoteAddr().String()+"]")
	wg.Done()
}
